    ...
    <PageTransition>
      <div className="min-h-screen bg-gray-900">
        <div className="max-w-4xl mx-auto px-4 py-6 sm:py-8">
          {/* Back */}
          <Link to="/" className="inline-flex items-center gap-2 text-gray-500 hover:text-white text-sm mb-4 sm:mb-6 transition-colors no-underline">
            <FiArrowLeft size={14} /> Dashboard
          </Link>

          {/* VS Header */}
          <AnimatedCard>
            <div className="bg-gradient-to-b from-gray-800 to-gray-800/50 border border-gray-700 rounded-2xl p-5 sm:p-8 mb-6 overflow-hidden relative">
              <div className="absolute inset-0 bg-gradient-to-r from-yellow-500/5 via-transparent to-purple-500/5 pointer-events-none" />
              <div className="flex items-center justify-center gap-4 sm:gap-8 relative">
                <div className="text-center">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-yellow-500/20 rounded-full flex items-center justify-center text-yellow-400 font-bold text-lg sm:text-2xl mx-auto ring-2 ring-yellow-500/20">
                    {user.username.charAt(0).toUpperCase()}
                  </div>
                  <p className="text-white font-semibold mt-2 text-sm sm:text-base">{user.username}</p>
                  <p className="text-green-400 text-xs sm:text-sm font-bold">{winsA} wins</p>
                </div>

                <div className="text-center px-2">
                  <div className="bg-gray-900/80 rounded-xl px-3 sm:px-4 py-2">
                    <p className="text-gray-500 text-2xl sm:text-3xl font-black tracking-wider">VS</p>
                  </div>
                  <p className="text-gray-600 text-[10px] sm:text-xs mt-1.5">{draws} draws</p>
                </div>

                <div className="text-center">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-purple-500/20 rounded-full flex items-center justify-center text-purple-400 font-bold text-lg sm:text-2xl mx-auto ring-2 ring-purple-500/20">
                ...
                <div className="text-center mt-5 sm:mt-6 relative flex flex-col sm:flex-row items-center justify-center gap-3">
                  ...
                  {!hasReachedTaskLimit && (
                    <Link
                      ...
                      className="inline-flex items-center gap-2 bg-yellow-500 hover:bg-yellow-400 text-gray-900 font-semibold px-4 sm:px-6 py-2.5 rounded-lg transition-all no-underline hover:shadow-lg hover:shadow-yellow-500/20 hover:scale-105 text-sm sm:text-base"
                    >
                      <FiPlus size={16} />
                  ...
                  <button
                    ...
                    disabled={disposing}
                    className="inline-flex items-center gap-2 bg-red-500/15 hover:bg-red-500/25 text-red-300 border border-red-500/30 font-semibold px-4 sm:px-5 py-2.5 rounded-lg transition-all text-sm sm:text-base disabled:opacity-60 cursor-pointer"
                  >
                    <FiTrash2 size={16} />
              ...
              {!isMyTurn && match.status === 'active' && (
                <div className="text-center mt-4 relative space-y-3">
                  <p className="text-gray-500 text-xs sm:text-sm">
                    {hasReachedTaskLimit
                      ? 'This match has reached its 5-task limit.'
                      : <>Waiting for <span className="text-purple-400 font-medium">{opponent?.username}</span> to create the next task...</>}
                  </p>
                  <button
                    type="button"
                    onClick={handleDisposeMatch}
                    disabled={disposing}
                    className="inline-flex items-center gap-2 bg-red-500/15 hover:bg-red-500/25 text-red-300 border border-red-500/30 font-semibold px-4 py-2 rounded-lg transition-all text-xs sm:text-sm disabled:opacity-60 cursor-pointer"
                  >
                    <FiTrash2 size={14} />
                    {disposing ? 'Disposing...' : 'Dispose Match'}
                  </button>
          ...
          <div className="mb-6">
            <h2 className="text-base sm:text-lg font-semibold text-white mb-3 sm:mb-4 flex items-center gap-2">
              ...
              Tasks
              <span className="text-xs bg-gray-700 text-gray-400 px-2 py-0.5 rounded-full">{tasks.length}/5</span>
            </h2>
            {tasks.length === 0 ? (
              <AnimatedCard>
                <div className="bg-gray-800 border border-dashed border-gray-600 rounded-xl p-8 sm:p-10 text-center">
          ...
          {results.length > 0 && (
            <div>
              <h2 className="text-base sm:text-lg font-semibold text-white mb-3 sm:mb-4 flex items-center gap-2">
                ...
                Results History
                <span className="text-xs bg-gray-700 text-gray-400 px-2 py-0.5 rounded-full">{results.length}</span>
              </h2>
              <div className="space-y-2">
                {results.map((r, i) => (
                  <AnimatedCard key={r._id} delay={i * 60}>
                    ...
                    >
                      <div className="bg-gray-800 border border-gray-700 rounded-lg p-3 sm:p-3.5 flex items-center justify-between hover:border-yellow-500/40 transition-all duration-300 hover:-translate-y-0.5">
                        <div className="min-w-0">
                          <p className="text-white text-sm font-medium truncate group-hover:text-yellow-300 transition-colors">
                            {r.taskId?.title || 'Task'}
                          </p>
                          <p className="text-gray-500 text-xs">{r.taskId?.date && new Date(r.taskId.date).toLocaleDateString()}</p>
                        </div>
                        <span
